...
class Moderator(commands.Cog):
    ...
    async def kick(self, ctx: commands.Context, user: nextcord.Member = None, reason: str = None):
        ...
        try:
            await user.kick(reason=f"You were kicked from **{ctx.guild}** for reason {reason}")
        except nextcord.Forbidden:
            await ctx.channel.send(f"Oops! I couldn't kick **{user}**. Please make sure he isn't a moderator.")
            return -1
        
        await ctx.channel.send(f"**{user}** was successfully kicked from **{ctx.guild.name}**")
        return 0
    ...
    async def masskick_error(self, ctx: commands.Context, error):
        ...
        if isinstance(error, BadArgument):
            embed = nextcord.Embed(title="Oops!", description="Something went wrong! (BadArgument)")
            await ctx.channel.send(embed=embed)
        elif isinstance(error, MissingPermissions):
            embed = nextcord.Embed(title="Oops!", description="You don't have the required permissions to run this command!")
            await ctx.channel.send(embed=embed)
    ...
    async def masskick_error(self, ctx: commands.Context, error):
        ...
        """
        if isinstance(error, BadArgument):
            embed = nextcord.Embed(title="Oops!", description=f"Something went wrong when trying to execute {ctx.command}! (BadArgument)")
            await ctx.channel.send(embed=embed)
        elif isinstance(error, MissingPermissions):
            embed = nextcord.Embed(title="Oops!", description="You don't have the required permissions to run this command!")
            await ctx.channel.send(embed=embed)
    ...
    async def ban(self, ctx: commands.Context, user: nextcord.Member = None, reason: str = None):
        ...
        if view is None:
            ...
            try:
                ...
                await ctx.channel.send(f"Oops! I couldn't ban **{user}** Please make sure he isn't a moderator.")
        else:
    ...
    async def massban(self, ctx: commands.Context, *args: nextcord.Member):
        ...
        if len(args) == 0:
            ...
            return 0
        confirm_embed = nextcord.Embed(title=random.choice(self.warning_messages), description=f"You're about to **permanently ban {len(args)}** users, are you sure?")
        await ctx.channel.send(embed=confirm_embed, view=view)
        ...
        if view is None:
            ...
            try:
                ...
                await ctx.channel.send(f"Alright! I successfully banned **{len(args)}** Users!")
            except nextcord.Forbidden:
    ...
    async def purge(self, ctx: commands.Context, num: int = None):  
        ...
        try:
            ...
            await ctx.channel.send(f"Successfully Deleted **{num}** Messages!", delete_after=5)
            
            return 0
        except nextcord.Forbidden:
            await ctx.channel.send("Sorry! I don't have enough permissions to delete messages!")
            return -1
